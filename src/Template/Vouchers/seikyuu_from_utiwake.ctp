<?php
	 ...
	 * @var \App\Model\Entity\ItemizedStatement[]|\Cake\Collection\CollectionInterface $itemizedStatements
	 */
...
<div class="row">
	<div class="col-6 breadcrumb_div">
		<ol class="breadcrumb m-b-20">
			...
			<li class="breadcrumb-item active">請求書生成</li>
		</ol>
	</div>
</div>
...
<div class="vouchers index large-9 medium-8 columns content">
	...
	<table cellpadding="0" cellspacing="0" class="table table-hover mb-0">
		...
		<tbody>
		<?php foreach ($itemizedStatements as $itemizedStatement): ?>
			<tr>
				<td class="align-middle"><?= h($itemizedStatement->customers_name) ?></td>
				<td class="align-middle"><?= h($itemizedStatement->delivery_dest) ?></td>
				<td class="align-middle"><?= h($itemizedStatement->created) ?></td>
				<td class="actions align-middle">
					<div class="custom-control custom-checkbox">
						<input type="checkbox" class="custom-control-input index_selecter" name="sel[]" value="<?= $itemizedStatement->id ?>"
						       id="customCheck<?= $itemizedStatement->id ?>">
						<label class="custom-control-label" for="customCheck<?= $itemizedStatement->id ?>">選択</label>
					</div>
				</td>
			</tr>
		<?php endforeach; ?>
				...
				<div class="card-body">
					...
					<div class="row my-2">
						<div class="col-12 text-center">
							<button class="btn btn-success mr-3" type="submit" name="submit_btn" value="seikyuu_hakkou">
								<i class="fe-edit"></i>
								<span>請求書を生成する</span>
							</button>
						</div>
